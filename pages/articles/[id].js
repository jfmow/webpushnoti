...
export default function Viewer({ arti }) {
    ...
    return <><SavedData savedData={articleData2} articleArti={artiArticle} /><Comments articleId={arti.article} /><ReportBtn articleName={artiArticle} /></>
}

const SavedData = ({ savedData, articleArti }) => {
    const [userSavedAlt, setUserSaved] = useState(false)
    const [shareModal, setShareModal] = useState(false);
    useEffect(() => {
        if (pb.authStore.isValid) {
            const fff = pb.authStore.model.saved_articles.includes(articleArti.id)
            setUserSaved(fff)
        }
    }, [articleArti])
