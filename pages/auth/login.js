...
export default function Login() {
  ...
  async function auth(event) {
    ...
    try {
      ...
      try {
        await getUserData(authData.record)
      } catch (error) {
        return window.location.replace(prevPage)
      }
      return window.location.replace(prevPage)
    } catch (error) {
      ...
      <div className={styles.login_box}>
        <div className={styles.card}>
          ...
          <form onSubmit={auth}>
            ...
            <p className={styles.signup} style={{ margin: '1em' }}>Please read the updated privacy policy!</p>
...
async function getUserData(urData) {
  console.log('Bot check...')
  if (urData.protection_data != true) {
    ...
    try {
      ...
      try {
        ...
        console.log('Updaing...')
        await pb.collection('private_data').create(privatedata);
        const data = {
          "protection_data": true
        };

        await pb.collection('users').update(urData.id, data);
      } catch (error) {
        console.log(error)
      }
    } catch (error) {
